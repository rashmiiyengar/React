...
const OrderConfirmationPage=()=>{
    ...
    useEffect(() => {
      ...
      // const ordersArray = orderItems.map((orderItem) => ({
    ...
    const handleOrderClick = (decodedData) => {
      ...
      dispatch(addToOrderHistory({ orders: orderDetails }));
    ...
    try{
      ...
      <div className="p-2 m-60 h-30 w-auto border-grey-50 border-b-4 text-center flex flex-col items-center justify-center shadow-lg" >
         ...
         <p className="font-semibold">Your order # {generateRandomOrderId()} is placed and will be delivered soon</p>
         <div>
         <p className="text-left m-3 text-gray-500 font-bold">ORDER ITEMS</p>
         <CartItem items={decodedData}/>
         </div>
         <button className='btn btn-outline-success' onClick={() => handleOrderClick(decodedData)}> Order History </button>
